...
THEME="bookmarks"
BOOKMARKS_FILE="$HOME/.config/rofi/bookmarks-list"
...
# Check if Zen browser is available in current workspace
focusedws=`i3-msg -t get_workspaces | jq '.[] | select(.focused==true).name' | cut -d"\"" -f2`
appinws1=`i3-msg -t get_tree | jq "recurse(.nodes[]) | select(.type==\"workspace\" and .num==$focusedws).nodes[] | select(.type==\"con\").nodes[].window_properties.class"`
appinws2=`i3-msg -t get_tree | jq "recurse(.nodes[]) | select(.type==\"workspace\" and .num==$focusedws).nodes[] | select(.type==\"con\").window_properties.class"`
appinws="$appinws1 $appinws2"

BROWSER="zen-browser --new-window"
for app in $appinws; do
   if [ "$app" = "\"zen-alpha\"" ]; then
...
INPUT=$(rofi -dmenu < "$BOOKMARKS_FILE")
