...
def get_action_key(a: Action):
    ...
    if isinstance(a, MirrorOf):
        return f"X Mirror of {a.target.name}"
...
def generate_xrandr_cmd(a: Action, active_output: list[Output]):
    ...
    if isinstance(a, MirrorOf):
        # select source resolution by default
        for m in a.target.modes:
            if m.preferred is True:
                resolution = f"{m.width}x{m.height}"
        return ["xrandr", "--output", a.output.name, "--mode", resolution, "--same-as", a.target.name]
...
def select_enable_action(connected_outputs, selected_output):
    ...
    for o in connected_outputs:
        if o.name != selected_output.name and o.isActive == True:
            ...
            actions.append(MirrorOf(selected_output, o))
