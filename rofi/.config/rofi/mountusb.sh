...
alreadymounted=$(lsblk -nrpo "name,type,mountpoint" | awk '$3!~/\/boot|\/home|\/opt$|SWAP/&&length($3)>1{printf "-not ( -path *%s -prune ) ",$3}')
...
unmountable=$(lsblk -nrpo "name,type,size,mountpoint,label" | awk -F':' '{gsub(/ /,":")}$4!~/\/boot|\/efi|\/home|\/opt$|SWAP/&&length($4)>1{printf "%s (%s) %s\n",$4,$3,$5}')
